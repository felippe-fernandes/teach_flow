...
CREATE POLICY "Users can view own data" ON users
  FOR SELECT
  USING (supabase_auth_id::uuid = auth.uid());
...
CREATE POLICY "Users can view own contractors" ON contractors
  FOR SELECT
  USING (user_id IN (SELECT id FROM users WHERE supabase_auth_id::uuid = auth.uid()));

CREATE POLICY "Users can insert own contractors" ON contractors
  FOR INSERT
  WITH CHECK (user_id IN (SELECT id FROM users WHERE supabase_auth_id::uuid = auth.uid()));
